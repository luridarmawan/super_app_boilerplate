...
/// - AUTH_TOKEN_REFRESH_URL: URL untuk refresh token (bisa mengandung {JWT} placeholder)
/// - AUTH_TOKEN_REFRESH_METHOD: HTTP method (GET atau POST, default: POST)
class TokenRefreshService {
  ...
  // Custom refresh URL (dapat di-set dari main app jika menggunakan AppInfo)
  String? _customRefreshUrl;
  String? _customRefreshMethod;

  /// Set custom refresh URL dan method
  /// Gunakan ini jika URL/method diambil dari sumber lain (bukan env vars)
  void configure({String? refreshUrl, String? refreshMethod}) {
    _customRefreshUrl = refreshUrl;
    _customRefreshMethod = refreshMethod;
  }
  ...
  Future<String?> refreshToken({String? currentJwt}) async {
    ...
    try {
      ...
      // Get refresh URL - prioritize custom config, then env variable
      String refreshUrl = _customRefreshUrl ?? '';
      ...
      // Get method - prioritize custom config, then env variable
      String method = _customRefreshMethod ?? '';
      if (method.isEmpty) {
